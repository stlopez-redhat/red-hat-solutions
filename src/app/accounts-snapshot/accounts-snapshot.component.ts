...
export class AccountsSnapshotComponent implements OnInit {
  ...
  accountInfo = new Array<ObAccountsData>();
  accounts = new Array<BankingAccountInfo>();
  obBalances = new Array<ObBalance>();
  selectedDebtAccount = new ObAccounts;
  selectedBalance = new Array<ObBalance>();
  ...
  beforePanelOpened(account: { AccountId: string; }){
    this.selectedBalance = this.obBalances.filter(balance => (balance.AccountId === account.AccountId));
  }

  accountSelected($event: { source: { value: ObAccounts; }; })  {
